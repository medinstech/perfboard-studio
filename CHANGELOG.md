...
Everything notable that changes in Perfboard Studio is written down here. The format follows
...
- **The application is called Perfboard Studio; its package and command are
  ...
  **The document format did not move.** `DOCUMENT_FORMAT_VERSION` is still 1, the
  extension is still `.perf` — it was always short for *perfboard*, never for the
  application — and every golden fixture in `tools/diffcheck/golden/` is byte-identical
  across the rename. A board written by v0.10.0 opens in this build unchanged, and the
  differential proof against the TypeScript engine is untouched.
...
- **Perfboard Studio can tell you a new version exists, and fetch it.** The last unwritten line
