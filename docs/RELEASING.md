...
**2. Bump the version.** In `src/perfboard/version.py`, drop the `.devN` suffix:
...
| | artefact | built on |
|---|---|---|
| PyPI | `perfboard-X.Y.Z-py3-none-any.whl` and the sdist | `ubuntu-latest` |
| Windows | `PerfboardStudio_vX.Y.Z_Setup.exe`, an NSIS installer | `windows-latest` |
| Linux | `perfboard-X.Y.Z-x86_64.AppImage` | `ubuntu-22.04`, deliberately |
| macOS | `perfboard-X.Y.Z-arm64.dmg` | `macos-15`, Apple silicon only |

Locally, on any of the three:

```sh
pip install -e ".[dev]"
python -m PyInstaller perfboard.spec --noconfirm      # -> dist/
bash packaging/appimage.sh                            # Linux
bash packaging/macos.sh                               # macOS
makensis packaging/perfboard.nsi                      # Windows (needs NSIS)
```
...
The application icon (`src/perfboard/ui/assets/`) is generated by `python
...
with its dependencies into a clean virtualenv and runs `perfboard --version` out of it.
...
to `pip install -U perfboard` rather than offering it a download.
...
> | field | value |
> |---|---|
> | PyPI Project Name | `perfboard` |
> | Owner | `medinstech` |
> | Repository name | `perfboard` |
> | Workflow name | `release.yml` |
> | Environment name | `pypi` |
>
> **Afterwards** — pypi.org → *Your projects* → **perfboard** → *Publishing* → **Add a
...
§12). The hooks are already in place for when that changes — `packaging/perfboard.nsi`
