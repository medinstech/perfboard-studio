...
**2. Bump the version.** In `src/perfboard_studio/version.py`, drop the `.devN` suffix:
...
running on. That line is what a bug report should quote — "which Perfboard Studio wrote this
file" and "can this Perfboard Studio read that file" are different questions, and the second
...
| | artefact | built on |
|---|---|---|
| PyPI | `perfboard_studio-X.Y.Z-py3-none-any.whl` and the sdist | `ubuntu-latest` |
| Windows | `PerfboardStudio_vX.Y.Z_Setup.exe`, an NSIS installer | `windows-latest` |
| Linux | `perfboard-studio-X.Y.Z-x86_64.AppImage` | `ubuntu-22.04`, deliberately |
| macOS | `perfboard-studio-X.Y.Z-arm64.dmg` | `macos-15`, Apple silicon only |

Locally, on any of the three:

```sh
pip install -e ".[dev]"
python -m PyInstaller perfboard-studio.spec --noconfirm  # -> dist/
bash packaging/appimage.sh                               # Linux
bash packaging/macos.sh                                  # macOS
makensis packaging/perfboard-studio.nsi                  # Windows (needs NSIS)
```
...
The application icon (`src/perfboard_studio/ui/assets/`) is generated by `python
...
with its dependencies into a clean virtualenv and runs `perfboard-studio --version` out of it.
...
to `pip install -U perfboard-studio` rather than offering it a download.
...
> | field | value |
> |---|---|
> | PyPI Project Name | `perfboard-studio` |
> | Owner | `medinstech` |
> | Repository name | `perfboard-studio` |
> | Workflow name | `release.yml` |
> | Environment name | `pypi` |
>
> **Afterwards** — pypi.org → *Your projects* → **perfboard-studio** → *Publishing* → **Add a
...
§12). The hooks are already in place for when that changes — `packaging/perfboard-studio.nsi`
