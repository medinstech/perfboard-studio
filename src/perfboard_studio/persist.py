...
def _num(path: str, n: int | float) -> int | float:
    ...
    if not math.isfinite(n):
        raise ValueError(
            ...
            f"Perfboard Studio documents must contain only finite numbers (a naive JSON "
...
def _parse_document(raw_input: object) -> tuple[PerfDocument, list[str]]:
    ...
    if format_version > CURRENT_FORMAT_VERSION:
        raise ValidationError(
            "format-too-new",
            f"This file was saved by a newer version of Perfboard Studio (file format {format_version}); "
            f"this build understands up to format {CURRENT_FORMAT_VERSION}. Please upgrade Perfboard Studio "
            f"to open it.",
            "formatVersion",
        )
