...
```sh
git clone https://github.com/medinstech/perfboard_studio.git
cd perfboard-studio
pip install -e ".[dev,mcp]"

pytest                 # the whole suite, ~2070 tests in under a minute
perfboard-studio       # launch the app on a blank board
```
...
`python -m perfboard_studio.ui.main --headless tools/diffcheck/golden/dense.perf` renders
...
**The engine is pure.** `src/perfboard_studio/` outside `ui/` and `mcp/` has no clock, no RNG,
...
- **Bugs and ideas:** open an issue. For a bug, `perfboard-studio --version` output and the
