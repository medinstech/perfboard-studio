...
import perfboard_studio
from perfboard_studio.model import DOCUMENT_FORMAT_VERSION
from perfboard_studio.version import (
    __version__,
    describe,
...
def test_package_reexports_the_version() -> None:
    """``perfboard_studio.__version__`` is what every other Python package trains people to try."""
    assert perfboard_studio.__version__ == __version__
...
def test_pyproject_derives_the_version_from_the_package() -> None:
    ...
    assert attr == "perfboard_studio.version.__version__"
...
def test_version_module_imports_nothing_at_module_scope() -> None:
    ...
    """
    source = (REPO_ROOT / "src" / "perfboard_studio" / "version.py").read_text(encoding="utf-8")
    module_scope = [
        line
