...
describe('DocumentStore', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'perfboard-studio-mcp-test-'));
  });
  ...
  it('falls back to the PERFBOARD_STUDIO_DOCUMENT env var when argv[2] is absent', () => {
    const path = join(dir, 'board.perf');
    writeFileSync(path, JSON.stringify(validDocument({ meta: { name: 'From env', created: 'x', modified: 'x' } })));

    const store = DocumentStore.load({ argv: ['node', 'stdio.js'], env: { PERFBOARD_STUDIO_DOCUMENT: path } });
  ...
  it('prefers argv[2] over the env var when both are given', () => {
    ...
    const store = DocumentStore.load({ argv: ['node', 'stdio.js', argvPath], env: { PERFBOARD_STUDIO_DOCUMENT: envPath } });
