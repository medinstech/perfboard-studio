 ...
 *   2. the PERFBOARD_STUDIO_DOCUMENT env var, if set.
...
import { DOCUMENT_FORMAT_VERSION, STANDARD_PITCH_MM } from '@perfboard-studio/core';
...
export class DocumentStore {
  ...
  static load(options: DocumentStoreOptions = {}): DocumentStore {
    ...
    const path = argPath !== undefined && argPath.length > 0 ? argPath : env['PERFBOARD_STUDIO_DOCUMENT'];
    ...
    log.info('No document path given (argv[2] / PERFBOARD_STUDIO_DOCUMENT); starting with an empty default board');
