...
imports inside a package. Going through this file keeps ``perfboard`` a real package at
runtime.

It exists only for the packaged builds. Running from source goes through the
``perfboard`` console script that ``pyproject.toml`` declares, which calls the same
``main``.
"""
...
def _ensure_streams() -> None:
    ...
    raises ``AttributeError`` on ``None.write`` -- and ``perfboard --version`` prints,
    so it would not print nothing, it would fall over. The command line has its own
    console build (``perfboard-cli.exe``); this is so that the windowed one cannot be
...
from perfboard.ui.main import main  # noqa: E402  - streams first, then imports
