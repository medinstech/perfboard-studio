...
> **Historical. The architecture this measures was not taken.** Perfboard Studio ships as a
...
Perfboard Studio ships as a Tauri v2 desktop app, which renders through the OS's
...
WebKitGTK's WebGL implementation can carry Perfboard Studio's 3D board scene. If it
can't, Perfboard Studio falls back to Electron on Linux (or everywhere). See
